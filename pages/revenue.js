...
const viewRevenue = (revenueAll, uid, startDate = 0, endDate = 0) => {
  clearDom();
  let revenueHTML = '';
  ...
  if (revenue.length) {
    revenueHTML += `
      <div>
        <div class="rev-text"><h1>${adminCheck(uid) ? 'REVENUE' : 'RECEIPTS'}</h1></div>
        <div class="rev-text"><h2 class="rev-text">TOTAL ${adminCheck(uid) ? 'REVENUE' : 'RECEIPTS'}: $${revenue.reduce((tot, rev) => tot + Number(rev.order_total), 0).toFixed(2)}</h2></div>
        <div class="details">
    ...
    revenueHTML += `
      <div>
        <h1>${adminCheck(uid) ? 'REVENUE' : 'RECEIPTS'}</h1>
        <h2>TOTAL ${adminCheck(uid) ? 'REVENUE' : 'RECEIPTS'}: $0.00</h2>
        <div class="rev-details">
          ${dateHTML}
        </div>
